...
9. **UI正本一致（必須）**：P0 全画面が `docs/ui-fidelity-contract.md` §3 チェックリストを満たし、**正本ワイヤー（design/ の Liquid Glass＋縦ぎり）と構造一致**。各UI PRに「スクショ↔ワイヤーPNG」の横並び比較を残す。
...
- **UIは正本ワイヤーから逸脱しない（最重要）**：`docs/ui-fidelity-contract.md` を厳守。Liquid Glass は機能層のみ／朱は時間のみ／色・フォントは DesignTokens 経由／**ワイヤーが先・実装が後**。全UI PRはスクショ↔ワイヤー比較＋§3チェックリストでゲート。
